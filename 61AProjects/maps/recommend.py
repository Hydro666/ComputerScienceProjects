...
def find_centroid(restaurants):
    ...
    "*** YOUR CODE HERE ***"
    pair = lambda x, f: [f(x[0]), f(x[1])] #apply function f to pair x
    return [pair(restaurant_location(r), mean) for r in restaurants]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        "*** YOUR CODE HERE ***"
        rest = group_by_centroid(restaurants, old_centroids)
        centroids = [find_centroid(r) for r in rest] 
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    def colR2(user, restaurants, feat_fn):                      #Returns R2 value of find_predictor
        return find_predictor(user, restaurants, feat_fn)[1]
    
    best_fn = max(feature_fns, key = lambda x: colR2(user, reviewed, x))
...
def rate_all(user, restaurants, feature_functions):
    ...
    "*** YOUR CODE HERE ***"
    revRes = user_reviewed_restaurants(user, restaurants)

    d = { #Picks the reviews associated with the reviewed restauraunts
    r : review_rating(user_reviews(user)[r]) for r in revRes
        } 

    for r in restaurants:
        if r not in d.keys():
            d[r] = predictor(restaurants[r])
    return d
