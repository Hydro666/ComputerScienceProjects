...
class ThrowerAnt(Ant):
    ...
    def distance(self, place):
        if self.place is place:
            return 0
        else:
            return distance(self, place.entrance) + 1
        ...
        while here != hive:
            if len(here.bees) > 0: 
                if count >= self.min_range and count <= self.max_range:
                    return random_or_none(here.bees)
                else:
                    here = here.entrance
                    count += 1    
            else:
...
class LaserAnt(ThrowerAnt):
    ...
    def distance(self, place):
        if self.place is place:
            ...
            return distance(place.entrance) + 1
    ...
    def action(self, colony):
        ...
        #         bee.reduce_armor(cur_dam)
        #         cur_dam -= self.insects_shot_weakening

        while cur_dam > 0 and cur_place is colony.hive:
            if cur_place.bees:
                for bee in cur_place.bees:
                    bee.reduce_armor(cur_dam)
                    cur_dam -= self.insects_shot_weakening
            cur_dam -= self.distance_weakening
...
class QueenPlace:
    ...
    def __init__(self, colony_queen, ant_queen):
        "*** YOUR CODE HERE ***"
        self.cq = colony_queen
        self.aq = ant_queen

    @property
    def bees(self):
        "*** YOUR CODE HERE ***"
        return self.cq.bees + self.aq.bees
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        #    print('and I would\'ve gotten away with it too if it weren\'t for you meddling programmers!')
            self.reduce_armor(self.armor)
            return

        colony.queen = QueenPlace(colony.queen, self.place)
        
        if self.place not in self.tunnel:
            self.tunnel.append(self.place)
        
        a, b = self.place, self.place
        
        while a.entrance:
            self.tunnel.append(a.entrance)
            a = a.entrance
        while b.exit:
            self.tunnel.append(b.exit)
            b = b.exit

        for place in self.tunnel:
            if place.ant and place.ant not in self.doubled and not isinstance(place.ant, QueenAnt):
...
class SlowThrower(ThrowerAnt):
    ...
    def make_slow(self, action, colony):
        if colony.time % 2 == 0:
            return self.action

    def apply_effect(self, effect, target, duration):
        target.action = self.effect
