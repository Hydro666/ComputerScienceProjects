...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        def helper(formal, val):
            ...
            if formal.second is nil:
                return None
            else:
                frame.define(formal.first, val.first)
                return helper(formal.second, val.second)
        helper(formals, vals)
...
def do_begin_form(expressions, env):
    ...
    return eval_all(expressions, env)   #Used to be return scheme_eval(expressions, env)
...
def do_and_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    if expressions is nil:
        return scheme_eval(True, env)
    def helper(exp):
        evl = scheme_eval(exp.first, env)
        if scheme_false(evl):
            return scheme_eval(False, env)
        elif exp.second is nil:
            return evl
        else:
            return helper(exp.second)
...
def make_let_frame(bindings, env):
    ...
    for pair in bindings:
        check_form(pair, 2, 2)
    formal = make_seq(bindings, 'first')
    check_formals(formal)
    prepreval = make_seq(bindings, 'second')
    preval = make_seq(prepreval, 'first')
    val = preval.map(lambda b: scheme_eval(b, env))
...
def check_formals(formals):
    ...
    while formals is not nil:
        if formals.first in lst or not scheme_symbolp(formals.first) or not scheme_listp(formals):
            raise SchemeError
